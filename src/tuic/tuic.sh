...
VERSION=$(wget -qO- https://raw.githubusercontent.com/pocomx/peace/master/version/tuic.txt | head -1 | tr -d [:space:])
wget -O $PWD/tuic https://github.com/pocomx/peace/releases/download/${VERSION}/${TUIC_FILE} > /dev/null 2>&1
if [ $? -ne 0 ]; then
