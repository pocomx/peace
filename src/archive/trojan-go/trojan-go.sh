...
VERSION=$(wget -qO- https://raw.githubusercontent.com/pocomx/peace/master/version/trojan-go.txt | head -1 | tr -d [:space:])
wget -O $PWD/trojan-go.zip https://github.com/pocomx/peace/releases/download/${VERSION}/${TROJAN_FILE} > /dev/null 2>&1
if [ $? -ne 0 ]; then
