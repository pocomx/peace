...
VERSION=$(wget -qO- https://raw.githubusercontent.com/pocomx/peace/master/version/v2ray.txt | head -1 | tr -d [:space:])
wget -O $PWD/v2ray.zip https://github.com/pocomx/peace/releases/download/${VERSION}/${V2RAY_FILE} > /dev/null 2>&1
if [ $? -ne 0 ]; then
