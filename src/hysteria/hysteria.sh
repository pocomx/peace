...
VERSION=$(wget -qO- https://raw.githubusercontent.com/pocomx/peace/master/version/hysteria.txt | head -1 | tr -d [:space:])
wget -O $PWD/hysteria.zip https://github.com/pocomx/peace/releases/download/${VERSION}/${HYSTERIA_FILE} > /dev/null 2>&1
if [ $? -ne 0 ]; then
